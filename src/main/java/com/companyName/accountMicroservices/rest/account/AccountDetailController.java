...
public class AccountDetailController {
    ...
    @RequestMapping(value = "/accountDetailBasicResponseParam",
            ...
            produces = MediaType.APPLICATION_JSON_VALUE)
    public @ResponseBody ResponseEntity<BasicResponse<List<AccountDetailResponse>>> accountDetailBasicResponseGET(@RequestParam String userCode) throws InvalidParameterException {

        log.info("Entering in accountDetail service(param)(JPA) - PathVariable: [{}]", userCode);

        List<AccountDetailResponse> delegateResult =  null;
        BasicResponse<List<AccountDetailResponse>> response = new BasicResponse<>();
        try {
            delegateResult= delegate.getAccountDetailJPA(userCode);
            if (!delegateResult.isEmpty() && delegateResult!=null){
                response.setData(delegateResult);
                //response.setTimestamp(fmt.format(new Date()));
            } else {
